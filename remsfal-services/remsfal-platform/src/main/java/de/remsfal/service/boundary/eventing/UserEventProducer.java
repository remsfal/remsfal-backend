...
public class UserEventProducer {
    ...
    public void sendUserUpdated(final UUID userId, final UserJson updatedProfile,
        final List<AffectedTenantJson> affectedTenants) {
        if (userId == null) {
            logger.warn("Skipping user event because userId is null");
            return;
        }
        final UserEventJson event = ImmutableUserEventJson.builder()
            ...
            .build();
        try {
            logger.infov("Sending user event (type={0}, userId={1})", UserEventType.USER_UPDATED, userId);
            final CompletionStage<Void> ack = emitter.send(event);
            ack.whenComplete((res, ex) -> {
                if (ex != null) {
                    logger.errorv(ex, "Failed to send user event (type={0}, userId={1})",
                        UserEventType.USER_UPDATED, userId);
                } else {
                    logger.infov("User event sent (type={0}, userId={1})", UserEventType.USER_UPDATED, userId);
                }
            });
        } catch (Exception e) {
            logger.errorv(e, "Error while sending user event (type={0}, userId={1})",
