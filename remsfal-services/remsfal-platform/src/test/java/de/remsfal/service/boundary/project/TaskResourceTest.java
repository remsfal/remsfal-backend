...
class TaskResourceTest extends AbstractResourceTest {
    ...
    void createTask_FAILED_userIsNotMember() {
        given()
            .when()
            .cookies(buildCookies(TestData.USER_ID_2.toString(), TestData.USER_EMAIL_2, Duration.ofMinutes(10)))
    ...
    void createTask_FAILED_userIsNotPrivileged() {
        ...
        given()
            .when()
            .cookies(buildCookies(TestData.USER_ID_2.toString(), TestData.USER_EMAIL_1, Duration.ofMinutes(10)))
            .contentType(MediaType.APPLICATION_JSON)
            .body("{ \"title\":\"" + TestData.TASK_TITLE + "\"}")
            .post(TASK_PATH, TestData.PROJECT_ID.toString())
            .then()
